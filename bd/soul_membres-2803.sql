...
--
-- Client :  localhost
-- Généré le :  Lun 28 Mars 2016 à 10:30
...
CREATE TABLE `droit_acces` (
  ...
  `acces_description` char(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(2, 'total', 'accès à toutes les fonctionnalités (sauf modif bd)'),
...
ALTER TABLE `se_connecter`
  ADD PRIMARY KEY (`mem_id`,`acces_id`,`mdp_id`),
  ADD KEY `FK_se_connecter_acces_id` (`acces_id`),
  ADD KEY `FK_se_connecter_mdp_id` (`mdp_id`);
...
--
ALTER TABLE `droit_acces`
  MODIFY `acces_id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
--
...
--
ALTER TABLE `mdp`
  MODIFY `mdp_id` int(11) NOT NULL AUTO_INCREMENT;
--
...
ALTER TABLE `se_connecter`
  ...
  ADD CONSTRAINT `FK_se_connecter_mdp_id` FOREIGN KEY (`mdp_id`) REFERENCES `mdp` (`mdp_id`),
